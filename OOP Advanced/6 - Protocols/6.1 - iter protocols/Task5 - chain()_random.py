# Реализуйте класс RandomLooper . При создании экземпляра класс должен
# принимать произвольное количество позиционных аргументов, каждый из
# которых является итерируемым объектом.
# Экземпляр класса RandomLooper должен являться итератором, который
# генерирует в случайном порядке все элементы всех итерируемых объектов,
...
class RandomLooper:
    ...
    def __init__(self, *iterables):
        self.iterables = list(it.chain(*iterables))
        self.start = 0
        self.end = len(self.iterables)
    ...
    def __next__(self):
        if self.start < self.end:
            self.start += 1
            return random.choice(self.iterables)
