# Реализуйте класс NonNegativeObject . При создании экземпляра класс должен
# принимать произвольное количество именованных аргументов. Все
# передаваемые аргументы должны устанавливаться создаваемому экземпляру в
# качестве атрибутов, причем если значением атрибута является отрицательное
# число, оно должно быть взято с противоположным знаком.
...
class NonNegativeObject:
    ...
    def __getattribute__(self, attr):
        if isinstance(object.__getattribute__(self, attr), (int, float)) and object.__getattribute__(self, attr) < 0:
            object.__setattr__(self, attr, -object.__getattribute__(self, attr))  
